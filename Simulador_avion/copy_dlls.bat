...
set OUTDIR=%~1
set VCPKG_ROOT=C:\Users\angel\Desktop\varios\vcpkg
...
if exist "%VCPKG_ROOT%\packages\glfw3_x64-windows\bin\glfw3.dll" (
    copy /Y "%VCPKG_ROOT%\packages\glfw3_x64-windows\bin\glfw3.dll" "%OUTDIR%" >nul 2>&1
)
...
if not exist "%OUTDIR%\glfw3.dll" (
    if exist "%VCPKG_ROOT%\installed\x64-windows\bin\glfw3.dll" (
        copy /Y "%VCPKG_ROOT%\installed\x64-windows\bin\glfw3.dll" "%OUTDIR%" >nul 2>&1
    )
)
